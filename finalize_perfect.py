...
if os.path.isdir(fotos_dir):
    for f_name in os.listdir(fotos_dir):
        if f_name.lower().endswith(('.jpg', '.jpeg', '.png')):
            ...
            n_esc = name.replace('"', '\\"')
            photos_data.append(f'  "{n_esc}": "data:{mime};base64,{b64}"')
...
for f_name in os.listdir('.'):
    if f_name.lower().endswith('.mp4'):
        ...
        n_esc = name.replace('"', '\\"')
        videos_data.append(f'  "{n_esc}": "data:video/mp4;base64,{b64}"')
...
# Using 80% for the video container inline width makes it smaller than 100%, but bigger than 52%.
new_SVG = '''let fallbackPhotoHTML = `<svg class="mem-photo-icon" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="8" width="34" height="24" rx="2.5" stroke="#4a3f30" stroke-width="1.4"/><circle cx="20" cy="20" r="6.5" stroke="#4a3f30" stroke-width="1.4"/><path d="M14 8l2-3h8l2 3" stroke="#4a3f30" stroke-width="1.4"/><circle cx="31" cy="13" r="1.5" fill="#4a3f30" opacity="0.4"/></svg><span class="mem-photo-label">Medya yok</span>`;
    ...
    for(let key in videosData) {
       let keyClean = key.toLocaleLowerCase('tr-TR').trim().replace(/i̇/g, 'i');
       if(contribClean === keyClean || contribClean.includes(keyClean) || keyClean.includes(contribClean)) { 
           foundVideo = videosData[key]; break; 
       }
    }
    ...
    if (foundVideo) {
        ...
        photo1HTML = `<img src="${foundImg}" alt="${contrib.name}" style="width:100%; height:auto; object-fit:contain; border-radius: 2px;" />`;
    }
'''
...
# 4. Inject inline style for just the video container logic
old_block_html = 'let blocksHTML = `<div class="mem-block"><div class="mem-photo"><div class="mem-photo-inner">${photo1HTML}</div></div><p class="mem-text">${contrib.text}</p><div class="mem-meta"><span class="mem-author">${contrib.name}</span></div></div>`;'
new_block_html = '''
    let vidStyle = foundVideo ? 'width:75%; max-width:400px;' : '';
    let blocksHTML = `<div class="mem-block"><div class="mem-photo" style="${vidStyle}"><div class="mem-photo-inner">${photo1HTML}</div></div><p class="mem-text">${contrib.text}</p><div class="mem-meta"><span class="mem-author">${contrib.name}</span></div></div>`;
'''
if old_block_html in html:
    html = html.replace(old_block_html, new_block_html)

# 5. Fix Önsöz Typo
html = html.replace("Hayatın her aşamasında bunu oku, hatırla ve bilir ki", "Hayatın her aşamasında bunu oku, hatırla ve bil ki;")

# 6. Apply CSS Overrides
css_fix = """
<style>
/* Un-crop images */
.mem-photo {
...
@media (max-width: 850px) {
  ...
  /* Original width for mobile should be larger than 52% */
  .mem-photo { width: 90%; }
